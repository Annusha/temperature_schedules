...
def nt_xent(x, t=0.5, features2=None, tau_adj=''):
    ...
    neg = torch.exp(torch.mm(out, out.t().contiguous()) / t_neg)
    ...
    t_pos = t
    # out = torch.cat([out_1, out_2], dim=0)
    pos = torch.exp(torch.sum(out_1 * out_2, dim=-1) / t_pos)
...
def cvt_state_dict(state_dict, bnNameCnt, in_features, num_classes):
    # deal with adv bn
    ...
    if bnNameCnt >= 0:
        ...
        for name in keys:
            if 'bn' in name:
                ...
                state_dict_new[name.replace('.bn_list.{}'.format(bnNameCnt), '')] = state_dict_new[name]
    ...
    if in_features is not None:
        ...
        for k, v in a.named_parameters():
            state_dict_new['fc.'+k] = v
        for k, v in a.named_buffers():
            state_dict_new['fc.' + k] = v
